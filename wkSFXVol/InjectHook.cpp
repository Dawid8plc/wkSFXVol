...
namespace HookLib {
    ...
    bool InstallHook(void* targetFunction, void* hookAddress, HookType type, bool appendNOP) {
        DWORD oldProtect;
        HookData data;

        // Save the original bytes of the target function.
        memcpy(data.originalBytes, targetFunction, 6);
        data.targetFunction = targetFunction;
        hooks[targetFunction] = data;

        // Change memory protection to read/write.
        VirtualProtect(targetFunction, 6, PAGE_EXECUTE_READWRITE, &oldProtect);
        ...
        switch (type) {
            case JMP_LONG: {
                ...
                *(DWORD*)((DWORD)targetFunction + 1) = relativeAddress;
                if(appendNOP)
				    *(BYTE*)((DWORD)targetFunction + 5) = 0x90; // NOP
                break;
            }
        ...
        VirtualProtect(targetFunction, 6, oldProtect, &oldProtect);
    ...
    bool RemoveHook(void* targetFunction) {
        ...
        DWORD oldProtect;
        VirtualProtect(targetFunction, 6, PAGE_EXECUTE_READWRITE, &oldProtect);
        memcpy(targetFunction, it->second.originalBytes, 6);
        VirtualProtect(targetFunction, 6, oldProtect, &oldProtect);
